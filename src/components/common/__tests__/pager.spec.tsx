...
import { RenderResult, cleanup, fireEvent } from 'react-testing-library';
...
describe('Pager', () => {
  it('disables next if there are no more pages', () => {
    ...
    const prev = getByText('PAGER_PREV');
    const next = getByText('PAGER_NEXT');
  ...
  describe('handles click events', () => {
    ...
    it('goes to next page on next click', () => {
      const next = result.getByText('PAGER_NEXT');

      fireEvent.click(next);
      expect(onChange).toBeCalledWith(3);
    });
    it('goes to prev page on prev click', () => {
      const prev = result.getByText('PAGER_PREV');
